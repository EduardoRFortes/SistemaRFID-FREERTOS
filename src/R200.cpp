...
void R200::acquireTransmitPower() {
  // Comando: AA 00 B7 00 00 B7 DD
  uint8_t checksum = (0x00 + 0xB7 + 0x00 + 0x00) & 0xFF;
  uint8_t buffer[] = {0xAA, 0x00, 0xB7, 0x00, 0x00, checksum, 0xDD};
  _serial->write(buffer, sizeof(buffer));
  Serial.println("Solicitada a potência de transmissão atual.");
}

void R200::sendCommand(uint8_t* command, uint8_t length) {
  _serial->write(command, length); // Enviando os dados via serial
}
